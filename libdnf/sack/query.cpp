...
Query::Impl::filterNevraStrict(int cmpType, const char **matches)
{
    ...
    if (compareSet.size() > 1) {
        ...
        while (true) {
            ...
            bool sameNEVRA = low->name == s->name && low->evr == s->evr && low->arch == s->arch;
            if (low != compareSet.end() && sameNEVRA) {
                MAPSET(&nevraResult, id);
            }
        }
    } else {
        for (auto & nevraId : compareSet) {
            Id id = -1;
            while (true) {
                id = result->next(id);
                if (id == -1)
                        break;
                Solvable* s = pool_id2solvable(pool, id);
                bool sameNEVRA = nevraId.name == s->name && nevraId.evr == s->evr &&
                    nevraId.arch == s->arch;
                if (sameNEVRA) {
                    MAPSET(&nevraResult, id);
                }
